...
export default function CargaStock() {
  ...
  const columns = [
    {
      dataIndex: "codigo",
      title: "Codigo",
      render: (_, reg) => <>{reg.codigo == "" ? "Pending..." : reg.codigo}</>,
    },
    ...
    if (e.keyCode === 13) {
      ...
      if (_id > -1) {
        ...
        if (typeof dict[_id.toString()] === "undefined") {
          ...
          setCodes((__codes) => [
            ...__codes,
            { id: _id, codigo: "", cantidad: 1, mensaje: "Pendiente..." },
          ]);
          ...
          fetch(get.detalle_stock + `${globals.obtenerSucursal()}/${_id}`)
            ...
            .then((response) => {
                ...
                fetch(get.detalle_codigo + response.data.idcodigo)
                  ...
                  .then((_response) => {
                    ...
                    if (_response.data.length > 0) {
                      setCodes((_c) => {
                        return _c.map((_c1) =>
                          _c1.id == _response.data[0].idcodigo
                            ? {
                                ..._c1,
                                codigo: _response.data[0].codigo,
                                mensaje: "Stock local no encontrado ",
                              }
                            : { ..._c1 },
                        );
                      });
              ...
              setCodes((_c1) => {
                return _c1.map((_c) =>
                  _c.id == response.data[0].idcodigo
                    ? {
                        ..._c,
                        codigo: response.data[0].codigo,
                        cantidad_actual: response.data[0].cantidad,
                        mensaje: "OK",
                      }
                    : { ..._c },
                );
          ...
          let _codes = codes.map((_c) =>
            _c.id == _id ? { ..._c, cantidad: _c.cantidad + 1 } : { ..._c },
          );
          setCodes((__) => _codes);
        }
      } else {
  ...
  const aplicar = () => {
    ...
    post_method(
      post.update.modificar_cantidad_lista,
      {
  ...
  const aplicar_filtros = (_) =>
    verSoloFaltantes ? srcControl.filter((r) => r.dif != 0) : srcControl;
    ...
    <>
      ...
      <Row>
        ...
        <Col span={8} style={{ padding: "1em" }}>
          ...
          <Row>
            <Col span={24}>{filtros()}</Col>
          </Row>
          <Row>
            <Col span={24} style={{ padding: "1em" }}>
              <Checkbox
                checked={verSoloFaltantes}
                onChange={() => {
                  setVerSoloFaltantes(!verSoloFaltantes);
                }}
              >
                Ver s&oacute;lo faltantes
              </Checkbox>
            </Col>
          </Row>
          <Row>
      ...
      <Row>
        <Col span={24}>
          ...
          <Input.TextArea
            ...
            maxLength={125}
            placeholder="Max. 125 Carácteres"
          ></Input.TextArea>
        </Col>
      </Row>
      <Row>
      ...
      <Row>
        <Col span={12}>
          <Button disabled={modoControl} block type="primary" onClick={aplicar}>
            Aplicar Cantidades y Guardar
          </Button>
        </Col>

        <Col span={12}>
          <Button
            disabled={!modoControl}
            block
            type="primary"
            onClick={guardar}
          >
