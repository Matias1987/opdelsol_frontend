...
export default function AgregarStockLoteV3(props) {
  ...
  const procesar_codigos = (_values) => {
    ...
    const pattern1 =
      /^([0-9A-Z_\.\s]+)\[([\-0-9\.]+)\s([\-0-9\.]+)\s([\-0-9\.]+)\]([0-9A-Z_\.\s]+)$/gm;

    const pattern2 =
      /^([0-9A-Z_\.\s]+)\[([\-0-9\.]+)\s([\-0-9\.]+)\s([\-0-9\.]+)\]([0-9A-Z_\.\s]+)\[([\-0-9\.]+)\s([\-0-9\.]+)\s([\-0-9\.]+)\]([0-9A-Z_\.\s]+)$/gm;
  ...
  const remove_row = (key) => {
    setTableData(tableData.filter((r) => r.codigo != key));
  };
  ...
  const columns = [
    { width: "150px", title: "Codigo", dataIndex: "codigo" },
    {
      ...
      render: (_, details) => <>{details.descripcion}</>,
    },
    ...
    { width: "150px", title: "Precio", dataIndex: "precio" },
    {
      width: "150px",
      title: "Etiquetas",
      render: (_, { tags }) => tags.map((t) => <Tag>{t}</Tag>),
    },
    {
      width: "150px",
      title: "Acciones",
              ...
              .then((_response) => {
                if (_response.data.length > 0) {
                  ...
                  if (values.length > 0) {
                    ...
                    alert("done");
                    document.location.href = public_urls.agregar_stock;
                  }
                } else {
                  /// crear stock
                  const _data = {
    ...
    <>
      ...
      >
        <Form onFinish={onFinish} form={form} onFinishFailed={onFinishFailed}>
          <Form.Item
            style={{ padding: "3.5em", fontSize: ".25em" }}
            label={""}
            name={"subgrupo"}
            rules={[{ required: true }]}
          >
              ...
              {subgrupo == null || +idSubgrupo < 0 ? (
                ...
                <p>
                  <div>
                    ...
                    >{`${subgrupo.familia} / ${subgrupo.subfamilia} / ${subgrupo.grupo} / `}</span>{" "}
                    <span
                      style={{
                        fontSize: "1.15em",
                        fontWeight: "bold",
                ...
                {subgrupo == null ? (
                  ...
                  <>
                    <Table
                      title={(_) => (
                        <>
                          C&oacute;digos a Generar&nbsp;&nbsp;
                          <Button
                            type="primary"
                            size="small"
                            onClick={onAgregarCodigosBtnClick}
                          >
          ...
          <Form.Item>
            <Button
              disabled={btnDisabled || (tableData || []).length < 1}
              type="primary"
              htmlType="submit"
            >
              Confirmar
      ...
      <Modal
        ...
        footer={null}
      >
        <PopUpAgregarStockLoteForm
          idsubgrupo={idSubgrupo}
          precioDefecto={
            typeof subgrupo?.precio_defecto === "undefined"
              ? 0
              : subgrupo?.precio_defecto
          }
          callback={(_data) => {
            agregarRow(_data);
            setPopupAddOpen(false);
