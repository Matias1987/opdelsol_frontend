        ...
        return permisos.map((p) => (
          <>
            <div style={{ margin: "2em", padding: "2px", fontSize: ".6em" }}>
              <b>{p.sucursal}&nbsp;</b>
              <Tag color={+p.ventas == 1 ? "green-inverse" : "red-inverse"}>
                Vtas
              </Tag>
              <Tag color={+p.caja1 == 1 ? "green-inverse" : "red-inverse"}>
                Caja
              </Tag>
              <Tag
                color={+p.deposito_min == 1 ? "green-inverse" : "red-inverse"}
              >
                DepMin
              </Tag>

              <Tag color={+p.deposito == 1 ? "green-inverse" : "red-inverse"}>
                Dep&oacute;sito
              </Tag>
              <Tag
                color={+p.laboratorio == 1 ? "green-inverse" : "red-inverse"}
              >
                Taller
              </Tag>

              <Tag color={+p.admin2 == 1 ? "green-inverse" : "red-inverse"}>
                Admin
              </Tag>
            </div>
          </>
  ...
  return (
    <>
      ...
      <Modal
        ...
        footer={null}
        title="Editar"
        open={popupPrivilegiosOpen}
        onCancel={() => {
          setPopupPrivilegiosOpen(false);
        }}
        key={selectedUsuario}
      >
        <AgregarPrivilegiosUsuarios
      ...
      <Modal
        destroyOnClose
        open={popupAddEditOpen}
        onCancel={() => {
          setPopupAddEditOpen(false);
        }}
        footer={null}
        title={editarUsuario ? "Editar" : "Agregar Usuario"}
      >
        <AgregarUsuarioForm
          idusuario={selectedUsuario}
          key={selectedUsuario}
          edicion={editarUsuario}
          callback={() => {
            setUpdate(!update);
            setPopupAddEditOpen(false);
          }}
