...
const AnularVentasCobradas = (props) => {
  ...
  const AnularVenta = () => {
    ...
    post_method(
      ...
      { idventa: props.idventa, estado: "ANULADO" },
      (resp) => {
       setCount(count + 1);//trigger update
      },
    );
  };
  ...
  const _venta_details = (_) => (
    <>
      ...
      <Row>
        <Col span={24}>Monto: {venta.monto_total}</Col>
      </Row>
      <Row>
        <Col span={24}>Vendedor: {venta.usuario_nombre}</Col>
      </Row>
      <Row>
        <Col span={24}>
