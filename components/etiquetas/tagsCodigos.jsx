...
const Tags = (props) => {
  ...
  const load = () => {
    post_method(post.lista_tag, { fkcategoria: "-1" }, (resp) => {
      setTags((_) =>
        resp.data.map((t) => ({ value: t.etiqueta, label: t.etiqueta })),
      );
    });

    post_method(
      post.lista_tags_codigo,
  ...
  return loading ? (
    ...
    <div
      style={{
        backgroundColor: "rgba(236, 252, 236, 1)",
        padding: ".75em",
        borderRadius: "8px",
      }}
    >
      <Row>
        <Col>
          <span>Etiquetas: </span>
          {codigo_tags.map((t) => (
            <Tag color="green-inverse" style={{ fontSize: "1.2em" }}>
              {t.nombre}
            </Tag>
          ))}
          {"1" === (props.readOnly || "1") ? (
