...
const InformeVentaV2 = (props) => {
  ...
  const callback_itemsLoaded = () => {
    setItemsLoaded(true);
  };
  const callback_sucursalLoaded = () => {
    setSucursalLoaded(true);
  };
  const callback_clienteLoaded = () => {
    setClienteLoaded(true);
  };
  const callback_destLoaded = () => {
    setDestLoaded(true);
  };
  const callback_MPLoaded = () => {
    setMPLoaded(true);
  };
  ...
  const productos = () => {
    ...
    switch (+data.tipo) {
      ...
      case 6:
        return (
          <LCLabItems idventa={data.idventa} callback={callback_itemsLoaded} />
        );
      case 3:
        return (
          <LCStockItems
                  ...
                  <tbody>
                          ...
                          <tbody>
                            <tr>
                              <td width="250px">
                                ...
                                >
                                  No se entregar&aacute;n trabajos sin esta
                                  boleta. Pasados los 30 d&iacute;as de la fecha
                                  estipulada los precios podr&aacute;n ser
                                  actualizados al d&iacute;a. Transcurridos los
                                  60 d&iacute;as a partir de la fecha, no se
                                  aceptar&aacute;n reclamos.
                                  <br />
                                </div>
                    ...
                    <tr>
                      <td style={{ padding: "0", fontSize: ".6em" }}>
                        <ScissorOutlined />
                        <hr style={{ margin: "-1em", border: "1px dotted" }} />
                        <br />
                      </td>
                    </tr>
                    <tr>
                          ...
                          <tbody>
                            <tr>
                              <td>
                                Vendedor: <b>{data.usuario_nombre}</b>
                                <ResponsableInf
                                  id={data.cliente_idcliente}
                                  callback={callback_clienteLoaded}
                                />
                                <DestinatarioInf id={data.fk_destinatario} />
                                <hr />
                                {data.obra_social != ""
                                  ? "Obra Social: " + data.obra_social
                                  : ""}
                                <br />{" "}
                                {data.medico != ""
                                  ? "Medico: " + data.medico
                                ...
                                <br />
                                Fecha de Entrega:{" "}
                                {data.fecha_entrega_formated +
                                  "  " +
                                  (data.hora_retiro == "null"
                                    ? "-"
                                    : data.hora_retiro)}
                              </td>
                              <td width="180px">
                                <div style={{ marginTop: "-1.2em" }}>
                                  <DataSucursalInf
                                    idsucursal={data.sucursal_idsucursal}
                                    callback={callback_sucursalLoaded}
                                  />
                                  <FechaEntregaInf data={data} />
                                </div>
                              </td>
