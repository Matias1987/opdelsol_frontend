...
export default function InformeCaja(props) {
  ...
  const data_sucursal = (_) =>
    dataSucursal == null ? <></> : <>Sucursal: {dataSucursal.nombre}</>;
  const header = (_) =>
    dataCaja == null ? (
      <></>
    ) : (
      <>
        <Row>
          <Col span={8}>{data_sucursal()}</Col>
          <Col span={8}>LISTADO DE CAJA DIARIA</Col>
          <Col span={8}>
            Fecha de emisi&oacute;n: {fecha} &nbsp;&nbsp;&nbsp; Hora de
            emisi&oacute;n: {hora}
          </Col>
        </Row>
        <Row>
          <Col span={24}>
            Caja: N1 Fecha: <b>{dataCaja.fecha_f}</b> &nbsp;&nbsp;&nbsp;Turno: M
            Estado: <b>{dataCaja.estado}</b> &nbsp;&nbsp;&nbsp;Inicio de Caja:{" "}
            <b>{dataCaja.monto_inicial}</b>
          </Col>
        </Row>
      </>
    );
    ...
    dataCaja == null ? (
      ...
      <>
        <Row>
          <Col span={6}>
            Total Ventas + Cuotas:&nbsp;
            {currency_format(totales.ventas + totales.cuotas)}&nbsp;
          </Col>
          <Col span={6}>
            Total Gastos:&nbsp;{currency_format(totales.gastos)}&nbsp;
          </Col>
          {/*<Col span={6}>Monto Transferido:&nbsp;{currency_format(totales.transferido)} &nbsp;Recibido:&nbsp;{totales.recibido}&nbsp;</Col>*/}
          <Col span={6}>
            NETO CAJA:&nbsp;
            {currency_format(
              parseFloat(totales.ventas) +
                parseFloat(totales.cuotas) -
                parseFloat(totales.gastos) -
                parseFloat(totales.transferido) +
                parseFloat(totales.recibido),
            )}
          </Col>
        </Row>
      </>
                ...
                <Table
                  ...
                  columns={[
                    ...
                    { title: "Recibo", dataIndex: "recibo" },
                    {
                      ...
                      title: "Ventas",
                      dataIndex: "efectivo",
                      render: (_, { efectivo }) => currency_format(efectivo),
                    },
                    {
                      align: "right",
                      title: "Cuotas",
                      dataIndex: "cuotas",
                      render: (_, { cuotas }) => currency_format(cuotas),
                    },
                    {
                      align: "right",
                      title: "Cheques",
                      dataIndex: "cheque",
                      render: (_, { cheque }) => currency_format(cheque),
                    },
                    {
                      align: "right",
                      title: "Tarjetas",
                      dataIndex: "tarjeta",
                      render: (_, { tarjeta }) => currency_format(tarjeta),
                    },
                    {
                      align: "right",
                      title: "Mutual",
                      dataIndex: "mutual",
                      render: (_, { mutual }) => currency_format(mutual),
                    },
                    {
                      align: "right",
                      title: "Cta.Cte.",
                      dataIndex: "ctacte",
                      render: (_, { ctacte }) => currency_format(ctacte),
                    },
                    {
                      align: "right",
                      title: "Mercado Pago",
                      dataIndex: "mercadopago",
                      render: (_, { mercadopago }) =>
                        currency_format(mercadopago),
                    },
                    {
                      align: "right",
                      title: "Transferencia",
                      dataIndex: "transferencia",
                      render: (_, { transferencia }) =>
                        currency_format(transferencia),
                    },
                  ]}
                  summary={(data) => {
                    var totalVentas = 0;
                      ...
                      <>
                        <Table.Summary.Row>
                          ...
                          <Table.Summary.Cell align={"right"}>
                            <b>{currency_format(totalVentas)}</b>
                          </Table.Summary.Cell>
                          <Table.Summary.Cell align={"right"}>
                            <b>{currency_format(totalCuotas)}</b>
                          </Table.Summary.Cell>
                          <Table.Summary.Cell align={"right"}>
                          ...
                          <Table.Summary.Cell align={"right"}>
                            <b>{currency_format(totalMercadoPago)}</b>
                          </Table.Summary.Cell>
                          <Table.Summary.Cell align={"right"}>
                            <b>{currency_format(totalTransferencias)}</b>
                          </Table.Summary.Cell>
                        </Table.Summary.Row>
                      </>
                    );
                ...
                <Table
                  ...
                  columns={[
                    ...
                    { title: "Detalle", dataIndex: "concepto_gasto" },
                    {
                      title: "Importe",
                      dataIndex: "monto",
                      render: (_, { monto }) => currency_format(monto),
                    },
                  ]}
                  summary={(data) => {
                    var total = 0;
