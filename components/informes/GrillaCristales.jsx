...
const GrillaCristales = (props) => {
    ...
    for (let i = 0; i < 1650; i += 25) {
      ...
      for (let j = 0; j < 750; j += 25) {
        ...
        if (i == 0 && j == 0) {
          ...
          if (i == 0) {
            msg = "-" + (parseFloat(j - 25) * 0.01).toString();
          } else {
            if (j == 0) {
              msg = (parseFloat(i - 25) * 0.01).toString();
            }
          }
        }
  ...
  useEffect(() => {
    ...
    fetch(
      get.lista_codigos_categoria +
        `${familia}/${subfamilia}/${grupo}/${subgrupo}/-1`,
    )
      ...
      <tr>
        {r.map((s) => (
          <td
            style={{
              ...curr_cell_style,
              backgroundColor: s.e == 1 ? "lightgreen" : "lightblue",
            }}
            onMouseEnter={(e) => {
