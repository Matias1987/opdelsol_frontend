  ...
  const columns = [
    {
      ...
      title: (
        <Checkbox
          ...
          onChange={(e) => {
            setDataSource((dt) =>
              dt.map((_sg) => ({ ..._sg, checked: e.target.checked }))
            );
          }}
        ></Checkbox>
      ),
        ...
        <Checkbox
          ...
          onChange={(_) => {
            setDataSource((ds) =>
              ds.map((sg) => ({
                ...sg,
                checked: sg.id == id ? !sg.checked : sg.checked,
              }))
            );
          }}
        ></Checkbox>
  ...
  return (
    <>
        ...
        <Row>
          <Col span={24}>
            <Button
              ...
              danger
              onClick={() => {
                setPopupEditSeleccionOpen(true);
              }}
            >
              Editar Selecci&oacute;n
            </Button>
            &nbsp;
          </Col>
        </Row>
                  ...
                  : dataSource.filter(
                      (r) =>
                        r.nombre_corto
                          .toString()
                          .toUpperCase()
                          .includes(filtroTabla.toUpperCase()) ||
                        r.nombre_largo
                          .toString()
                          .toUpperCase()
                          .includes(filtroTabla.toUpperCase())
      ...
      <Modal
        ...
        width={"70%"}
        onCancel={() => {
          setPopupEditSeleccionOpen(false);
        }}
        footer={null}
      >
        <EditarLoteSubgrupo
          subgrupos={dataSource
            .filter((sg) => sg.checked)
            .map((sg) => ({
              nombre_corto: sg.nombre_corto,
              idsubgrupo: sg.id,
            }))}
          callback={() => {
            setChange(!change);
