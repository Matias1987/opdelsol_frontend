...
const ChatV2 = (props) => {
  ...
  return (
    <>
    <Card style={{width:"700px"}} title={<>Chat</>}>
      <Row>
          <Col
            ref={containerRef}
            span={24}
            style={{ height: "400px", overflow: "scroll" , padding:"8px"}}
            onScroll={(e) => {}}
          >
            <List
                ...
                <List.Item>
                  <List.Item.Meta
                    style={{
                      padding:"8px",
                      backgroundColor:
                        item.fkemisor == uid ? "lightyellow" : "white",
                      textAlign: item.fkemisor == uid ? "right" : "left",
                    }}
                    avatar={
                      <Avatar
                        src={`https://xsgames.co/randomusers/avatar.php?g=pixel&key=${item.fkemisor}`}
                      />
                    }
                    ...
                    description={
                      <>
                        ...
                        &nbsp;&nbsp;
                        <i style={{ fontSize: ".7em" }}>{item.fecha}</i>
                      </>
                    }
                  />
                </List.Item>
        ...
        <Row>
          <Col span={24}>
            <Input
              ...
              onChange={(e) => {
                setMessageToSend((m) => ({ ...m, mensaje: e.target.value }));
              }}
              onKeyDown={(e) => {
                if (e.key === "Enter") {
              ...
              style={{
                backgroundColor: "#CAFF7A",
                width: "100%",
                padding: "1em",
              }}
              value={messageToSend.mensaje}
              prefix={""}
              addonAfter={
                <>
                  <Button
