...
const CodeGrid = (props) => {
  ...
  const load_ejes_if_any = () => {
    post_method(
      ...
      { idsubgrupo: props.idsubgrupo, idsucursal: props.idsucursal, eje: "-1" },
      (response) => {
        let t_ejes = {};
        let _ejes = [];
        let _min_esf = 9999;
  ...
  const load = () => {
    ...
    post_method(
      tipoGrilla == "p"
        ? post.obtener_uso_items_adic_subgrupo_periodo
        : post.obtener_grilla_stock,
      {
        idsubgrupo: props.idsubgrupo,
        idsucursal:
          (props.idsucursal || "-1") == "-1"
  ...
  const draw = () => {
    ...
    let d = 0;
    for (let esf = min_esf, i = 0; esf <= max_esf; esf += 0.25, i++) {
      for (let cil = min_cil, j = 0; cil <= max_cil; cil += 0.25, j++) {
        if (
          cols[`${cil.toFixed(2)}`] == 0 &&
          rows[`${(esf > 0 ? "+" : "") + esf.toFixed(2)}`] == 0
        ) {
        ...
        if (j == 0) {
          ctx.fillText(
            `  ${esf.toString()}  `,
            0,
            tileh * (i + 1) + tileh / 2 + yoffset,
          );
        }
        if (i == 0) {
          ctx.fillText(
            ` ${cil.toString()} `,
            tilew * (j + 1) + (8 - cil.toString().length) + xoffset,
            tileh / 2,
          );
        }
        ...
        if (dict[idx] != null) {
          ctx.fillStyle = dict[idx].mouseover
            ? "#75DBA3"
            : defaultCellFillStyle;
          ...
          //TEXT
          ctx.font =
            dict[idx].cantidad > 0 || dict[idx].stock_ideal > 0
              ? `12px Arial`
              : `10px Arial`;
          ctx.fillStyle =
            dict[idx].cantidad > 0 || dict[idx].stock_ideal > 0
          ...
          switch (tipoGrilla) {
            case "c":
              ctx.fillText(dict[idx].idcodigo.toString(), x + 6, y + 14);
              break;
            case "s":
            ...
            case "p":
              ...
              break;
            case "i":
              ctx.fillText(dict[idx].stock_ideal.toString(), x + 6, y + 14);
              break;
            case "d":
              ctx.fillText(
                (
                  parseInt(dict[idx].cantidad) - parseInt(dict[idx].stock_ideal)
                ).toString(),
                x + 6,
                y + 14,
              );
    ...
    ctx.rect(xoffset2 - xoffset * m, yoffset2 - yoffset * m, rect_w, rect_h);
  ...
  const mark_selected_rect = () => {
    let selection = [];
    let xstart =
      selection_rect.x < selection_rect.x1
        ? selection_rect.x
        : selection_rect.x1;
    let ystart =
      selection_rect.y < selection_rect.y1
        ? selection_rect.y
        : selection_rect.y1;
    ...
    for (let i = xstart; i < xend + 1; i++) {
      for (let j = ystart; j < yend + 1; j++) {
        ...
        if (dict[map[idx]] != null) {
          selection.push({
            idcodigo: dict[map[idx]].idcodigo,
            codigo: dict[map[idx]].codigo,
          });
        }
      }
    }
  ...
  const getCSVText = (_) => {
    ...
    _csvtext += "NEGATIVOS\r\n";
    _csvtext += "ESF\\CIL,";
    for (let cil = max_cil; cil >= min_cil; cil -= 0.25) {
      _csvtext += `${cil.toString()},`;
    }
    _csvtext += "\r\n";

    for (let esf = -0.25, i = 0; esf >= min_esf; esf -= 0.25, i++) {
      for (let cil = max_cil, j = 0; cil >= min_cil; cil -= 0.25, j++) {
        let text = "0";

        if (j == 0) {
          _csvtext += `${esf.toString()},`;
        }
        ...
        if (dict[idx] != null) {
          switch (tipoGrilla) {
            case "c":
              text = dict[idx].idcodigo.toString();
              break;
            case "s":
              text = dict[idx].cantidad.toString();
              break;
            case "p":
              text = dict[idx].cantidad.toString();
              break;
            case "i":
              text = dict[idx].stock_ideal.toString();
              break;
            case "d":
              text = (
                parseInt(dict[idx].cantidad) - parseInt(dict[idx].stock_ideal)
    ...
    for (let esf = 0, i = 0; esf <= max_esf; esf += 0.25, i++) {
      for (let cil = max_cil, j = 0; cil >= min_cil; cil -= 0.25, j++) {
        let text = "0";
    ...
    <>
      ...
      >
        <EditarStockIndiv
          ...
          idcodigo={selectedCode?.idcodigo || "-1"}
    ...
    <>
      {selectedCode != null && codigosSeleccion.length < 1 ? (
        <>
          ...
          <Modal
            footer={null}
            open={popupStockIdeal}
            onCancel={() => {
              setPopupStockIdeal(false);
            }}
          >
            <CargaStockIdeal
  ...
  const periodo_mode = (_) => (
    <>
      <Row>
        <Col span={24}>Consumo Por Periodo</Col>
      </Row>
      <Row>
        <Col span={24}>
      ...
      <Row>
        <Col span={24}>
          <Button
            disabled={filtroPeriodo.desde == "" && filtroPeriodo.hasta == ""}
            size="small"
            onClick={() => {
              setReload(!reload);
            }}
  ...
  return (
    <>
      ...
      <Row>
        <Col span={24}>
          ...
          {subgrupo == null ? (
            ...
            <>
              <Tag color="geekblue">{subgrupo.nombre_largo}</Tag>
            </>
          )}
          <br />
          <br />
            ...
            <>
              <Row>
                <Col span={24}>
                  ...
                  &nbsp;&nbsp;Cantidad:{" "}
                  <b>
                    {tipoGrilla == "s" || tipoGrilla == "p"
                      ? selectedCode.cantidad
                      : tipoGrilla == "i"
                        ? selectedCode.stock_ideal
                        : parseInt(selectedCode.cantidad) -
                          parseInt(selectedCode.stock_ideal)}
                  </b>
                </Col>
              </Row>
            </>
      ...
      <Row>
        <Col span={24}>
          ...
          <EditarCodigoGrupo
            ...
            codigos={codigosSeleccion.map((c) => ({
              idcodigo: c.idcodigo,
              codigo: c.codigo,
              precio: c.precio,
            }))}
      ...
      <Row>
        <Col span={12}>Cantidad total:&nbsp;{total}</Col>
        <Col span={12}>
          {subgrupo == null ? (
            <></>
          ) : (
            <ExportToCSV
              parseFnt={(_) => subgrupo.nombre_largo + "\r\n" + csvText}
              key={csvText}
            />
          )}
        </Col>
