...
const EditarSobre = (props) => {
        ...
        <>
          ...
          {record.idcodigo > 0 ? (
            <CustomModal openButtonText="Detalle">
              {" "}
              <DetalleCodigo idcodigo={record.idcodigo} />{" "}
            </CustomModal>
          ) : (
            <></>
          )}
          ...
          {record.items.map((i) => (
            <>
              <Tag
                ...
                style={{ fontSize: i.closable ? "1em" : ".85em" }}
                color={i.closable ? "red" : "purple"}
              >
                {i.codigo}
              </Tag>
            </>
                ...
                onClose={() => {
                  ...
                  let _ur = usedRows.map((r1) =>
                    record.tipo == r1.tipo
                      ? {
                          ...r1,
                          pedidos: r1.pedidos.filter(
                            (r2) => r2.localId != p.localId,
                          ),
                        }
                      : r1,
                  );
  ...
  const save = (_) => {
    ...
    usedRows.forEach((row) => {
      _data_items_adicionales = [
        ..._data_items_adicionales,
        ...row.items.filter((it) => it.userAdded),
      ];

      _data_pedidos = [
        ..._data_pedidos,
        ...row.pedidos.filter((p) => p.userAdded),
      ];
    });
  ...
  const populate_rows = (data) => {
    ...
    data.forEach((data_row) => {
      if (+data_row.original == 1) {
        _rows = _rows.map((_row) =>
          _row.tipo == data_row.tipo
            ? {
                ..._row,
                codigo: data_row.codigo,
                isadicional: false,
                idcodigo: data_row.idcodigo,
                agregarEnabled: true,
                usarEnabled: true,
              }
            : _row,
    ...
    post_method(
      post.obtener_items_ventas_taller,
              ...
              : {
                  ...
                  pedidos: [
                    ...
                    ...[
                      {
                        codigo: cod.codigo,
                        fkcodigo: cod.idcodigo,
                        tipo: modifyingId,
                        userAdded: true,
                        localId: idlocalPedidos,
                      },
                    ],
                  ],
                },
      ...
      <>
        <Row>
          <Col span={24} style={{ padding: "1em" }}>
            <Row>
              <Col span={24}>
                <span style={{ color: "#0016dd" }}>
                  Detalle Venta Nro.: {props.idventa}
                </span>{" "}
                {+props.idtrabajo > 0 ? (
                  <span style={{ color: "#ee0000ff", fontWeight: "600" }}>
                    {" "}
                    - Trabajo id: {props.idtrabajo}
                  </span>
                ) : (
                  <></>
                )}
            ...
            <Row style={{ padding: "1em" }}>
              ...
              <Col span={4}>
                ...
                <MostrarDNI dni={venta.cliente_dni} />{" "}
              </Col>
            ...
            <Row style={{ padding: "1em" }}>
              ...
              <Col span={16}>
                <b>{venta.fecha_entrega_formated}</b>{" "}
              </Col>
            </Row>
          </Col>
        </Row>
          ...
          <Row gutter={16}>
            <Col>
              <Button
                danger
                disabled={
                  !btnCambiarEstadoEnabled ||
                  props.readonly ||
                  venta?.estado_taller == "CALIBRADO"
                }
                size="small"
                onClick={on_marcar_calibrado_click}
              >
            ...
            <Col>
              <Button
                ...
                disabled={
                  !btnCambiarEstadoEnabled ||
                  props.readonly ||
                  venta?.estado_taller == "LAB"
                }
                size="small"
                onClick={on_marcar_como_terminado_click}
              >
            ...
            <Col>
              <Button
                ...
                disabled={
                  ...
                  props.readonly ||
                  venta?.estado_taller == "LAB" ||
                  venta?.estado_taller == "PEDIDO"
                }
                size="small"
                onClick={on_marcar_laboratorio_click}
              >
                Laboratorio
              </Button>
            </Col>
          ...
          <Row gutter={16}>
            <Col>
              <Button
                type="primary"
                onClick={save}
                disabled={!btnSaveEnabled || props.readonly}
              >
                Aplicar Cambios
              </Button>
            </Col>
