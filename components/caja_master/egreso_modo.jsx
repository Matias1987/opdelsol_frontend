...
import { Button, Col, InputNumber, Modal, Row, Select, Table } from "antd";
  ...
  const loadCuentasBancarias = (_) => {
    ...
    fetch(get.obtener_cuenta_bancarias)
      ...
      .then((response) => {
        ...
        setCuentasBancarias(
          response.data.map((cb) => ({
            value: cb.id_cuenta_bancaria,
            label: cb.alias,
            idCuenta: cb.id_cuenta_bancaria,
          })),
        );
      })
      .catch((error) => {
  ...
  const onAgregarClick = (_) => {
    ...
    const modo = {
      ...
      selectedCuenta: selectedCuenta
        ? cuentasBancarias.find((cb) => cb.idCuenta == selectedCuenta)[0] ||
          null
        : null,
      localId: localId,
    };
              ...
              { title: "Monto", dataIndex: "monto" },
              {
                ...
                render: (text, record) => (
                  <Button
                    type="link"
                    onClick={() => {
                      const _temp = egresoModos.filter(
                        (em) => em.localId != record.localId,
                      );
                      calcularTotal(_temp);
        ...
        <Row style={rowStyle}>
          <Col span={24}>
            <Button
              disabled={
                0 == monto ||
                ("tb" == selectedModo && null == selectedCuenta) ||
                !selectedModo
              }
              type="primary"
              block
