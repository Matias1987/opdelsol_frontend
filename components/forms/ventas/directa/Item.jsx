...
const VDItem = (props) => {
  ...
  return (
    <>
      <Button
        style={{ marginBottom: "8px", borderRadius: "8px", fontWeight: "600" }}
        size="middle"
        block
        type="primary"
        onClick={() => {
      ...
      <Table
        ...
        columns={[
          ...
          { title: "Desc.", dataIndex: "descripcion", width:"120px" },
          {
            ...
            width: "160px",
            render: (_, { idcodigo, cantidad_max, cantidad }) => (
              <>
                <Input
                  type="number"
                  min={1}
                  ...
                  onChange={(e) => {
                    onCantidadChange(
                      parse_int_string(e.target.value) > cantidad_max
                        ? cantidad_max
                        : parse_int_string(e.target.value),
                      idcodigo,
                    );
                  }}
                />
          ...
          },
          {
            ...
            dataIndex: "precio",
            width: "160px",
            render: (_, { precio, codigo, precio_str }) => (
              <Input
                onClick={(e) => {
                  e.target.select();
                }}
                ...
                onChange={(e) => {
                  setItems((__items) => {
                    const _items_ = __items.map((i) =>
                      i.codigo == codigo
                        ? {
                            ...i,
                            precio_str: e.target.value,
                            precio: parse_float_string(e.target.value),
                            total:
                              parse_float_string(e.target.value) *
                              parse_float_string(i.cantidad),
                          }
                        : i,
                    );
