...
const CambiarResponsableDestinatario = (props) => {
        ...
        fetch(get.cliente_por_id + resp.data[0].cliente_idcliente)
          ...
          .then((response) => {
            setResponsable({
              ...
              nombre:
                response.data[0].apellido + ", " + response.data[0].nombre,
            });
  ...
  const onGuardar = () => {
    ...
    if (cambios.iddestinatario != -1) {
      post_method(
        post.update.cambiar_destinatario,
        { idventa: props.idventa, iddestinatario: cambios.iddestinatario },
        (response) => {
    ...
    if (cambios.idresponsable != -1) {
      post_method(
        post.update.cambiar_responsable,
        { idventa: props.idventa, idresponsable: cambios.idresponsable },
        (response) => {
      ...
      >
        <>
          ...
          {responsableLoading ? (
            ...
            <>
              <Row style={{ backgroundColor: "rgba(0,250,0,.15)" }}>
                <Col span={24}>
                  Responsable Actual: {responsable.nombre}
                  &nbsp;&nbsp;&nbsp;D.N.I.: {responsable.dni}
                </Col>
              </Row>
            </>
          )}
          {destinatarioLoading ? (
            ...
            <Row style={{ backgroundColor: "rgba(0,250,0,.15)" }}>
              <Col span={24}>
                Destinatario Actual: {destinatario.nombre}
                &nbsp;&nbsp;&nbsp;D.N.I.: {destinatario.dni}
              </Col>
            </Row>
          )}
          <Row>
          ...
          <Row>
            <Col span={24}>
              <Button
                onClick={onGuardar}
                disabled={
                  !(cambios.iddestinatario > 0 || cambios.idresponsable > 0)
                }
              >
                Guardar Cambios
              </Button>
