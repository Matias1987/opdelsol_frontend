  ...
  const onFinish = () => {
    ...
    switch (props.action) {
      case "ADD":
        setBtnEnabled(false);
        post_method(
          post.insert.subgrupo,
          { ...subgrupo, uid: postIdRef.current },
          (res) => {
            setBtnEnabled(true);
            if (res.status == "OK") {
  ...
  return (
    <>
      ...
      <Row style={_row_style}>
        <Col span={24}>
          <Input
            style={_input_style}
            prefix={"Nombre Corto"}
            value={subgrupo.nombre_corto}
            onChange={(e) => {
              setSubgrupo((sg) => ({
                ...sg,
                nombre_corto: e.target.value.toUpperCase(),
              }));
            }}
          />
        </Col>
      ...
      <Row style={_row_style}>
        <Col span={24}>
          <Input
            style={_input_style}
            prefix={"Nombre Largo"}
            value={subgrupo.nombre_largo}
            onChange={(e) => {
              setSubgrupo((sg) => ({
                ...sg,
                nombre_largo: e.target.value.toUpperCase(),
              }));
            }}
          />
        </Col>
      ...
      <Row style={_row_style}>
        <Col span={24}>
          <Input
            style={{ ..._input_style, width: "310px", textAlign: "right" }}
            prefix={"Multiplicador"}
            type="number"
            step={".1"}
            min="0"
            value={subgrupo.multiplicador}
            onChange={(e) => {
              setSubgrupo((sg) => ({ ...sg, multiplicador: e.target.value }));
            }}
          />
        </Col>
      </Row>
      <Row style={_row_style}>
        <Col span={24}>
          <Input
            prefix={"Precio por Defecto"}
            style={{ ..._input_style, width: "310px", textAlign: "right" }}
            type="number"
            step={".1"}
            value={subgrupo.precio_defecto}
            min="0"
            onChange={(e) => {
              setSubgrupo((sg) => ({ ...sg, precio_defecto: e.target.value }));
            }}
          />
        </Col>
      </Row>
      ...
      <Row style={{ ..._row_style, textAlign: "right" }}>
        <Col span={24}>
          <Button onClick={onFinish} type="primary" disabled={!btnEnabled}>
            Agregar Subgrupo
          </Button>
        </Col>
      </Row>
