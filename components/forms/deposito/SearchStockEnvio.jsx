...
const SearchStockEnvio = (props) => {
  ...
  const doSearch_old = (value, id) => {
    ...
    const url =
      search_url +
      props.idSucursalDestino +
      "/" +
      encodeURIComponent(_searchvalue) +
      "/" +
      id +
      "/" +
      categoria +
      ...
      .then((_response) => {
        ...
        setDataSource(
          response.data.map((row) => ({
            ...
            habilitado: {
              val:
                typeof row.habilitado === "undefined" ? true : row.habilitado,
              idcodigo: row.idcodigo,
            } /* un codigo puede no estar habilitado */,
            checked: false,
          })),
        );
        setLoading(false);
  ...
  const doSearch = (value, id) => {
    ...
    if (+idCat < 1 && _searchvalue.length < 1) {
      setLoading(false);
      return;
    }
    ...
    post_method(post.buscar_stock_envios, params, (response) => {
      ...
      setDataSource(
        response.data.map((row) => ({
          ...
          habilitado: {
            val: typeof row.habilitado === "undefined" ? true : row.habilitado,
            idcodigo: row.idcodigo,
          } /* un codigo puede no estar habilitado */,
          checked: false,
        })),
    ...
    <>
      <Card
        title={
          ...
          >
            Agregar C&oacute;digos al Env&iacute;o
          </div>
        }
        size="small"
        style={{
          ...
          borderRadius: "16px",
          borderColor: "rgba(208, 216, 243, 0.9)",
        }}
      >
        <>
          <Row gutter={16}>
            <Col>
              <Checkbox
                checked={conStockOrigen}
                onChange={() => {
                  setConStockOrigen(!conStockOrigen);
                }}
              >
                Con stock origen
              </Checkbox>
            ...
            <Col>
              <Checkbox
                checked={sinStockDestino}
                onChange={() => {
                  setSinStockDestino(!sinStockDestino);
                }}
              >
                Sin stock destino
              </Checkbox>
              ...
              <Table
                ...
                columns={[
                  ...
                  { width: "150px", title: "Desc.", dataIndex: "descripcion" },
                  {
                    ...
                    dataIndex: "cantidad",
                    render: (_, { cantidad }) => (
                      <span style={{ color: "#00972D" }}>{cantidad}</span>
                    ),
                  },
                  {
                    width: "80px",
                    title: "Dest.",
                    dataIndex: "cantidad_destino",
                    render: (_, { cantidad_destino }) => (
                      <span style={{ color: "red" }}>{cantidad_destino}</span>
                    ),
                  },
                  {
                    width: "50px",
                        ...
                        <>
                          {habilitado.val ? (
                            <Button
                              size="small"
                              onClick={() => {
                                props.callback([habilitado.idcodigo]);
                              }}
                            >
                              <PlusOutlined /> Agregar
                            </Button>
